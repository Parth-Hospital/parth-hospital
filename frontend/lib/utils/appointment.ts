...
export function generateTimeSlots(
  ...
  for (let hour = startHour; hour < endHour; hour++) {
    for (let minute = 0; minute < 60; minute += slotDuration) {
      ...
      const bookedInSlot = Math.max(
        0,
        Math.min(existingBookings - slotStartNumber + 1, patientsPerSlot),
      );
...
export function getNextAppointmentDate(): Date | null {
  ...
  const isWindowOpen =
    currentTime >= windowOpenTime || currentTime < windowCloseTime;
...
export async function checkDoctorAvailability(date: Date): Promise<boolean> {
  try {
    const { doctorAvailabilityApi } =
      await import("@/lib/api/doctorAvailability");
    const availability = await doctorAvailabilityApi.getAvailability(
      date.toISOString(),
    );
...
export async function isBookingWindowOpen(): Promise<{
  ...
  if (qaMode) {
    ...
    return {
      ...
      doctorAvailabilityMessage: `[QA MODE] Dr. Ashish Bharadwaj is available for consultations tomorrow (${formatDate(tomorrow)}).`,
      isBefore5PM: false,
    };
  }
  ...
  if (isBefore5PM && isAfter8_15AM) {
    ...
    return {
      isOpen: false,
      message:
        "Booking window will open at 5 PM today. Doctor availability will be updated before then.",
      nextDate: tomorrow,
  ...
  if (!nextDate) {
    return {
      isOpen: false,
      message:
        "Booking window is closed. It opens at 5 PM the day before your appointment.",
      nextDate: null,
      doctorAvailable: false,
      doctorAvailabilityMessage: "",
      isBefore5PM: false,
  ...
  if (!doctorAvailable) {
    return {
      ...
      doctorAvailable: false,
      doctorAvailabilityMessage:
        "Dr. Ashish Bharadwaj is not attending tomorrow. Please check back later for future appointments.",
      isBefore5PM: false,
    };
  }
  ...
  return {
    ...
    nextDate,
    doctorAvailable: true,
    doctorAvailabilityMessage: `Dr. Ashish Bharadwaj is available for consultations tomorrow (${formatDate(nextDate)}).`,
    isBefore5PM: false,
  };
}
...
export function validateAppointmentForm(data: Partial<AppointmentFormData>): {
  ...
  if (
    !data.patientAge ||
    isNaN(Number(data.patientAge)) ||
    Number(data.patientAge) < 1 ||
    Number(data.patientAge) > 120
  ) {
  ...
  if (
    !data.phoneNumber ||
    !/^[6-9]\d{9}$/.test(data.phoneNumber.replace(/\D/g, ""))
  ) {
